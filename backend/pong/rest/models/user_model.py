...
class User(models.Model):
	...
	oauth_username = models.CharField(max_length=50, blank=True, null=True, unique=True, db_index=True)
	...
	@staticmethod
	def fetch_oauth_user(username:str):
		try:
			user = User.objects.filter(oauth_user=True, oauth_username=username).values('id').get()

...
class CookieAuth(BaseAuthentication):
    def authenticate(self, request):
        ...
        if not token:
            return AuthenticationFailed("No Cookie Was Given")
        user_uuid = parse_uuid([token])
        if len (user_uuid) != 1:
            return AuthenticationFailed("Wrong UUID")
        db_user = UserInfo.fetch_users_by_id(user_uuid)
        if len(db_user != 1):
            return AuthenticationFailed("Couldn't find a user with specific UUID")

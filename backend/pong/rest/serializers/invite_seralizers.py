...
class InviteSerializer(serializers.Serializer):
	...
	def accept_game_invite(invited: UserSerializer, invite_id):
		...
		invite  = InviteSerializer(db_invite[0])
		if (invite.data['game']['game_started'] == True or invite.data['invited']['id'] != invited.data['id'] or 
			(invite.data['seen'] == True and invite.data['accepted'] == False) or  invite.data['type'] != INVITE_TYPE[0][0] 
			or invite.data['accepted'] == True
			):
	...
	@staticmethod
	def	fetch_game_invites(invited:UserSerializer):
		available_invites = Invite.fetch_active_invites(invited.instance)
		return InviteSerializer(available_invites, many=True)
...
from ..serializers.game_seralizers import GameSerializer, GAME_TYPES, GAME_GENRE
from ..serializers.invite_seralizers import InviteSerializer
from ..helpers import OsnierMatchmaking , parse_uuid
import json
import random

class OsnierMatchmakingSocket(WebsocketConsumer):
	def connect(self):
	...
	def disconnect(self, code):
		OsnierMatchmaking().remove_player(self.channel_name)
		return super().disconnect(code)
	
	def join_game_matchmaking(self):
		game_queue = OsnierMatchmaking().add_player(self.channel_name)
	...
	def game_match(self, event):
		...
		game = GameSerializer.create_new_game(self.scope['user'], GAME_TYPES[1][0], GAME_GENRE[1][0])

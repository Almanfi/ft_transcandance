...
class MessagingSocket(WebsocketConsumer):
    ...
    def handle_friendship_message(self, data):
        ...
        friendship = Relationship.get_relationship_between(self.scope.user, friend[0])
        if len(friendship) != 1 or friendship[0].data['type'] != RELATIONSHIP_STATUS[1][0]:
        ...
        return {"status": "Sent", "message": new_message.content}
